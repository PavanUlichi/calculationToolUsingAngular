...
(function () {
  ...
  app.controller('CalculatorController', ['$scope', function($scope){
    ...
    $scope.calc = function(){
      ...
      $scope.inc_cost = ((($scope.inc_wattage*$scope.current_hours*$scope.total_days)/1000)*cost).toFixed(2);
      $scope.hal_cost = ((($scope.hal_wattage*$scope.current_hours*$scope.total_days)/1000)*cost).toFixed(2);
      $scope.cfl_cost = ((($scope.cfl_wattage*$scope.current_hours*$scope.total_days)/1000)*cost).toFixed(2);
      $scope.led_cost = ((($scope.led_wattage*$scope.current_hours*$scope.total_days)/1000)*cost).toFixed(2);
      }
    $scope.calc();
